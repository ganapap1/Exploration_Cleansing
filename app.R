...
server <- function(input, output, session) {
    ...
    showModal(
      if(length(vmy$df_types[input$dt_cell_clicked$row,1])>0 ){
        ...
        if (sapply(vmy$mydata[n], is.Date)== TRUE){
          ...
          modalDialog(
            title = "Warning",
            selectInput(inputId = 'mDateFormat',label = 'Select Date format of your Dataset:',
                        choices = c('dmy','ymd'),selected = 'dmy'),
  ...
  observeEvent(input$mDateyes, {
    ...
    mselected <- vmy$df_types[clicked_list,1]
    for (i in mselected){
      if (input$mDateFormat=='dmy'){
        vmy$mydata[,i] <-lubridate::dmy(vmy$mydata[,i])
      }else{
        vmy$mydata[,i] <-lubridate::ymd(vmy$mydata[,i])
      }
